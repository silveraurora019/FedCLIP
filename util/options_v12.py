...
def args_parser(return_parser=False): # (保持 v11 GPU 修复方案)
    ...
    parser.add_argument('--non_iid_prob_class', type=float, default=0.3, help="non iid sampling prob for class")
    ...
    # === v9/v10/v11 的参数 (v12 继承) ===
    parser.add_argument('--k_clusters', type=int, default=1, help="Number of sub-prototypes per class for K-Means clustering (for L_PKD).")
    parser.add_argument('--base_threshold', type=float, default=0.8, help="Base cosine similarity threshold (for v9) or CLIP confidence threshold (for v11/v12).")
    parser.add_argument('--dynamic_threshold_factor', type=float, default=0.1, help="Factor to modulate the dynamic confidence threshold (v9 only).")
    ...
    parser.add_argument('--lambda_pkd', type=float, default=0.5, help="Weight for the Prototype Knowledge Distillation loss (L_PKD).")
    parser.add_argument('--temp_pkd', type=float, default=1.0, help="Temperature for L_PKD.")
