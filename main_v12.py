...
# --- (v11 GPU 修复方案) ---
from util.options_v12 import args_parser # 1. 导入 v12
parser = args_parser(return_parser=True) # 2. 获取 v12 parser
...
# 5. 导入 v12 组件
from util.local_training_v12 import LocalUpdate, globaltest
...
"""
Version 12: FedPKD (v11) + Prompt Ensembling
- Based on v11 (精简版 + CLIP Oracle).
- v12 Innovation:
  - Replaces the single-prompt text prototype generation
    with "Prompt Ensembling".
  - In 'clip_manager_v12.py', we now use a list of 7-8
    different prompt templates for each class.
  - The features for these templates are averaged to create
    a much more robust and less-biased text prototype.
- This further enhances the accuracy of the CLIP Noise Oracle
  with minimal extra cost (only at initialization).
- The training loop (L_CE + L_PKD + L_Distill) remains unchanged.
"""
...
if __name__ == '__main__':
    ...
    # 7. 更新日志路径 (移除了 clip_prompt)
    txtpath = rootpath + 'txtsave/V12_CLIP_Ensemble_%s_%s_NL_%.1f_LB_%.1f_Rnd_%d_E_%d_Frac_%.2f_LR_%.3f_Seed_%d' % (
    ...
    f_acc.write("="*50 + "\n")
    f_acc.write("Training Parameters (V12 - CLIP Prompt Ensembling):\n")
    ...
    if 'resnet' not in args.model:
        print("Warning: v12's L_PKD works best with ResNet-like features.")

    max_accuracy = 0.0

    # ============================ Training Loop (与 v11 相同) ============================
    print("\n" + "="*25 + " Stage: FedAvg with V12 Local Loss (CLIP Ensemble Oracle) " + "="*25, flush=True)
    final_accuracies = []
